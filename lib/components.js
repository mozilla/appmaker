...
function addLocalComponents() {
  components = fs.readdirSync("public/bundles").filter(function(dirName) {
    return !isComponentExcluded(dirName);
  }).map(function(dirName) {
    return path.join("bundles", dirName, 'component.html');
  }).filter(function(fileName) {
    return fs.existsSync(path.join("public", fileName));
  });
  return components;
}
