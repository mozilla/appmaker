...
function checkTag (tree, tagName, attributes) {
  ...
  Object.keys(attributes).forEach(function (name) {
    ...
    if (! inputAttribute) {
      throw 'Attribute "' + name + '" does not exist for ' + tagName;
    }
...
function filterApp (tree) {
  ...
  if (tree._childNodes) {
    siftThroughChildren(tree._childNodes, filterCardOrLink);
  }
  else {
    throw "No children!";
  }
...
function filterLinks (tree) {
  checkTag(tree, 'link', { type: "text/ceci" });
  return true;
}

function filterCardOrLink (tree) {
  if (tree.attributes['id'] && tree.attributes['id'].value == 'hooks') {
    siftThroughChildren(tree, filterLinks);
    return true;
  } else {
  ...
  filter: function (html, callback) {
    ...
    jsdom.env(html, {
        ...
        try {
          ...
          console.error(e);
          console.log("Exception doing validation, returning nothing");
          callback('', e);
        }
      }
    });
